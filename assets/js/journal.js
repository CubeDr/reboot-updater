...
(function () {
  ...
  function coachCard(r) {
    ...
    return '' +
      '<article class="jr-item" data-id="' + esc(r.id) + '">' +
        ...
        '</div>' +
        '<div class="jr-item__actions">' +
          '<button class="jr-mini jr-mini--danger" data-del type="button">일지 삭제</button>' +
        '</div>' +
      '</article>';
  }
  ...
  function bindCoachActions() {
    $all('.jr-item', coachListEl).forEach(function (card) {
      ...
      var delBtn = $('[data-del]', card);
      if (delBtn) delBtn.addEventListener('click', function () {
        var row = coachRows.filter(function (x) { return x.id === id; })[0];
        var who = row ? (row.member_name || '회원') : '회원';
        if (!confirm(who + ' 님의 이 일지를 삭제할까요?\n삭제하면 되돌릴 수 없습니다.')) return;
        delBtn.disabled = true;
        sb.from('journals').delete().eq('id', id).then(function (res) {
          if (res.error) { alert('삭제 실패: ' + res.error.message); delBtn.disabled = false; return; }
          coachRows = coachRows.filter(function (x) { return x.id !== id; });
          renderCoachStats();
          renderCoachFilter();
          renderCoachList();
        });
